...
use lingshu_autonomy::experience::{
    ExperienceEntry, ExperienceOutcome, ExperienceSeverity, ExperienceStore, ExperienceType,
};
        ...
        let entries: Vec<ExperienceEntry> = (0..100)
            .map(|i| {
                ExperienceEntry::new(
                    &format!("agent-{}", i % 5),
                    match i % 7 {
                        0 => ExperienceType::TaskExecution,
                        1 => ExperienceType::Decision,
                        2 => ExperienceType::Conversation,
                        3 => ExperienceType::Error,
                        4 => ExperienceType::Performance,
                        5 => ExperienceType::Feedback,
                ...
                .with_severity(match i % 5 {
                    0 => ExperienceSeverity::Info,
                    1 => ExperienceSeverity::Notice,
                    2 => ExperienceSeverity::Warning,
                    3 => ExperienceSeverity::Error,
...
fn bench_reflection(c: &mut Criterion) {
    ...
    group.bench_function("reflect_100_entries", |b| {
        ...
        b.iter(|| rt.block_on(async { black_box(engine.reflect("agent-r").await) }))
    });
...
fn bench_evolution(c: &mut Criterion) {
    ...
    group.bench_function("evolve", |b| {
        ...
        b.iter(|| rt.block_on(async { black_box(engine.evolve("agent-e").await) }))
    });
