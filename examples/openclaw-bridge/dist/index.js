...
class ConsoleChannel {
    ...
    async sendMedia(to, mediaUrl, text) {
...
class TelegramChannel {
    ...
    async sendText(to, text, replyTo) {
        ...
        if (replyTo)
            params.reply_to_message_id = Number(replyTo);
...
class FeishuChannel {
    ...
    async sendText(to, text, replyTo) {
        ...
        const result = await this.callApi(`/open-apis/im/v1/messages?receive_id_type=open_id`, params);
        return {
            success: true,
...
async function main() {
    ...
    // 创建 MCP 服务器
    const server = new Server({ name: "openclaw-bridge", version: "1.0.0" }, { capabilities: { tools: {} } });
    // ── tools/list ──
        ...
        try {
            switch (name) {
                ...
                case "channel_send_text": {
                    const { channel: chId, to, text, reply_to } = args;
                    const ch = channels.get(chId);
                    if (!ch)
                        throw new Error(`Channel "${chId}" not found`);
                ...
                case "channel_send_media": {
                    const { channel: chId, to, media_url, text } = args;
                    const ch = channels.get(chId);
                    if (!ch)
                        throw new Error(`Channel "${chId}" not found`);
                ...
                case "channel_send_payload": {
                    const { channel: chId, to, text, media_urls, is_error } = args;
                    const ch = channels.get(chId);
                    if (!ch)
                        throw new Error(`Channel "${chId}" not found`);
                    const payload = {
                    ...
                    const receipt = await ch.sendPayload(to, payload);
                    return {
                        content: [{ type: "text", text: JSON.stringify(receipt) }],
                    };
                }
                case "channel_health": {
                    const { channel: chId } = (args ?? {});
                    if (chId) {
                        const ch = channels.get(chId);
                        if (!ch)
                            throw new Error(`Channel "${chId}" not found`);
                        ...
                        return {
                            content: [{ type: "text", text: JSON.stringify({ [chId]: status }) }],
                        };
                    }
                    // 检查所有通道
                    const results = {};
