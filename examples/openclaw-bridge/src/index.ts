...
async function main() {
  ...
  if (HTTP_PORT) {
    ...
    const sseClients: Array<import("node:http").ServerResponse> = [];
    ...
    const httpServer = http.createServer(
      async (req: import("node:http").IncomingMessage, res: import("node:http").ServerResponse) => {
        ...
        if (pathname === "/api/tools/list" && req.method === "GET") {
          const handler = (server as any)["_requestHandlers"]?.tools?.list;
          if (handler) {
        ...
        if (pathname === "/api/tools/call" && req.method === "POST") {
          ...
          req.on("end", async () => {
            try {
              ...
              const handler = (server as any)["_requestHandlers"]?.tools?.call;
              if (handler) {
    ...
    httpServer.listen(parseInt(HTTP_PORT, 10), "127.0.0.1", () => {
      console.error(
        `[INFO] openclaw-bridge HTTP server on http://127.0.0.1:${HTTP_PORT}`,
      );
    });
  }
}
