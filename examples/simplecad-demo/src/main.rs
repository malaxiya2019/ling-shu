...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let make_box = tools
        .iter()
        .find(|t| t.info().name == "cad_make_box")
        .unwrap();
    ...
    println!(
        "   📦 创建基板: {}",
        serde_json::to_string(&box_result).unwrap_or_default()
    );

    // 2. 创建圆柱孔
    let make_cyl = tools
        .iter()
        .find(|t| t.info().name == "cad_make_cylinder")
        .unwrap();
    let cyl_result = make_cyl
        .execute(
            ctx.clone(),
            serde_json::json!({
    ...
    println!(
        "   🕳️  创建圆柱: {}",
        serde_json::to_string(&cyl_result).unwrap_or_default()
    );

    // 3. 创建第二个圆柱
    let cyl2_result = make_cyl
        .execute(
            ctx.clone(),
            serde_json::json!({
    ...
    println!(
        "   🏗️  创建凸台: {}",
        serde_json::to_string(&cyl2_result).unwrap_or_default()
    );

    // 4. 获取信息
    let get_info = tools
        .iter()
        .find(|t| t.info().name == "cad_get_info")
        .unwrap();
    let info_result = get_info
        .execute(ctx.clone(), serde_json::json!({"shape_tag": "base_plate"}))
        .await?;
    println!(
        "   ℹ️  基板信息: {}",
        serde_json::to_string(&info_result).unwrap_or_default()
    );
