...
fn route_permission(method: &axum::http::Method, path: &str) -> RoutePermission {
    match (method.as_str(), path) {
        // 健康检查 — 无需认证
        ("GET", "/health") => RoutePermission { permission: "", skip_auth: true },

        // Agent 管理
        ("GET", "/api/v1/agents") => RoutePermission { permission: permissions::agent::LIST, skip_auth: false },
        ("POST", "/api/v1/agents") => RoutePermission { permission: permissions::agent::CREATE, skip_auth: false },
        ("GET", p) if p.starts_with("/api/v1/agents/") && !p.contains("/action") => {
            RoutePermission { permission: permissions::agent::GET, skip_auth: false }
        }
        ("DELETE", p) if p.starts_with("/api/v1/agents/") => {
            RoutePermission { permission: permissions::agent::DELETE, skip_auth: false }
        }
        ("POST", p) if p.starts_with("/api/v1/agents/") && p.contains("/action") => {
            RoutePermission { permission: permissions::agent::ACTION, skip_auth: false }
        }

        // 会话管理
        ("GET", "/api/v1/sessions") => RoutePermission { permission: permissions::session::LIST, skip_auth: false },
        ("POST", "/api/v1/sessions") => RoutePermission { permission: permissions::session::CREATE, skip_auth: false },
        ("DELETE", p) if p.starts_with("/api/v1/sessions/") => {
            RoutePermission { permission: permissions::session::DELETE, skip_auth: false }
        }

        // 工具
        ("GET", "/api/v1/tools") => RoutePermission { permission: permissions::tool::LIST, skip_auth: false },
        ("POST", p) if p.starts_with("/api/v1/tools/") && p.contains("/execute") => {
            RoutePermission { permission: permissions::tool::EXECUTE, skip_auth: false }
        }

        // 工作流
        ("GET", "/api/v1/workflows") => RoutePermission { permission: permissions::workflow::LIST, skip_auth: false },
        ("POST", p) if p.starts_with("/api/v1/workflows/") && p.contains("/execute") => {
            RoutePermission { permission: permissions::workflow::EXECUTE, skip_auth: false }
        }
        ("GET", p) if p.starts_with("/api/v1/workflows/") && p.contains("/status") => {
            RoutePermission { permission: permissions::workflow::STATUS, skip_auth: false }
        }

        // Runtime
        ("GET", "/api/v1/runtime/status") => RoutePermission { permission: permissions::runtime::STATUS, skip_auth: false },
        ("GET", "/api/v1/runtime/health") => RoutePermission { permission: permissions::runtime::HEALTH, skip_auth: false },
        ("GET", "/api/v1/runtime/config") => RoutePermission { permission: permissions::runtime::CONFIG, skip_auth: false },
        ("GET", "/api/v1/runtime/stats") => RoutePermission { permission: permissions::runtime::STATS, skip_auth: false },

        // 实时事件流 — 需要会话认证
        ("GET", "/api/v1/events") => RoutePermission { permission: permissions::runtime::STATUS, skip_auth: false },
        ("GET", "/api/v1/ws") => RoutePermission { permission: permissions::runtime::STATUS, skip_auth: false },

        _ => RoutePermission { permission: "", skip_auth: true },
    }
}
        ...
        Box::pin(async move {
            ...
            let token = match extract_bearer_token(&req) {
                ...
                None => {
                    ...
                    let resp = (StatusCode::UNAUTHORIZED, 
            ...
            let required_perm = match Permission::parse(route_perm.permission) {
                Ok(p) => p,
                Err(_) => {
                    warn!("auth: invalid permission format '{}' for {}", route_perm.permission, path);
                    let resp = (StatusCode::INTERNAL_SERVER_ERROR,
                        serde_json::to_string(&serde_json::json!({
            ...
            let granted_perms: Vec<Permission> = auth_result.roles.iter()
                .map(|role| Permission::new("runtime", role, "access"))
                .collect();

            if let Err(e) = layer.permission_checker.check(&granted_perms, &required_perm) {
                warn!("auth: permission denied for user {} on {}: {}",
                    auth_result.user_id, path, e);
                let resp = (StatusCode::FORBIDDEN,
                    serde_json::to_string(&serde_json::json!({
            ...
            info!("auth: granted {} access to {} (roles: {:?})",
...
/// 启动 HTTP 服务器.
pub async fn serve(runtime: AgentRuntime, addr: SocketAddr) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
fn extract_auth_context(req: &axum::extract::Request) -> LsContext {
    if let Some(auth) = req.extensions().get::<AuthContext>() {
        ...
        let mut ctx = LsContext::with_session(session_id)
            .with_user(&auth.user_id);
        if let Some(ref tid) = auth.tenant_id {
            ctx = ctx.with_tenant(tid);
        }
        // 角色列表存入 metadata
        ctx.metadata.insert("roles".to_string(), auth.roles.join(","));
        ctx
    } else {
...
async fn sse_events(
    ...
    if let Some(bus) = runtime.event_bus().await {
        ...
        tokio::spawn(async move {
            let _ = bus.subscribe(ctx, "ls.*", Box::new(move |event| {
