     ...
     $src_flag:expr, $out_flag:expr, $out_name:expr, $run_cmd:expr) => {
        ...
        impl LanguageRunner for $name {
            ...
            async fn run(&self, code: &str, config: &PolyglotConfig) -> PolyglotResult<String> {
                ...
                if !$src_flag.is_empty() { comp.arg($src_flag); }
                comp.arg(src_path.to_string_lossy().as_ref());
                if !$out_flag.is_empty() { comp.arg($out_flag); }
                comp.arg(out_path.to_string_lossy().as_ref());
                execute_command(&mut comp, config).await
...
compiled_runner!(CRunner, Language::C, ".c", "gcc", "", "-o", "main_out", "./main_out");
compiled_runner!(CppRunner, Language::Cpp, ".cpp", "g++", "", "-o", "main_out", "./main_out");
compiled_runner!(SwiftRunner, Language::Swift, ".swift", "swiftc", "", "-o", "main_out", "./main_out");
compiled_runner!(AdaRunner, Language::Ada, ".adb", "gnatmake", "", "-o", "prog", "./prog");
compiled_runner!(CobolRunner, Language::Cobol, ".cob", "cobc", "-x", "-o", "prog", "./prog");
compiled_runner!(DelphiRunner, Language::Delphi, ".pas", "fpc", "", "-o", "prog", "./prog");
...
impl LanguageRunner for ZigRunner {
    ...
    fn language(&self) -> Language { Language::Zig }
    ...
    async fn run(&self, code: &str, config: &PolyglotConfig) -> PolyglotResult<String> {
        ...
        comp.arg("build-exe").arg(src.to_string_lossy().as_ref())
            .arg("--name").arg("prog");
        execute_command(&mut comp, config).await
            .map_err(|e| PolyglotError::CompilationFailed(e.to_string()))?;
        let mut run = Command::new(&out);
...
impl LanguageRunner for HaskellRunner {
    fn language_name(&self) -> &'static str { "Haskell" }
    fn language(&self) -> Language { Language::Haskell }
    fn file_extension(&self) -> &'static str { ".hs" }
    async fn run(&self, code: &str, config: &PolyglotConfig) -> PolyglotResult<String> {
        let dir = tempfile::tempdir()?;
...
impl LanguageRunner for ObjectiveCRunner {
    fn language_name(&self) -> &'static str { "Objective-C" }
    fn language(&self) -> Language { Language::ObjectiveC }
    ...
    async fn run(&self, code: &str, config: &PolyglotConfig) -> PolyglotResult<String> {
        ...
        comp.arg(src.to_string_lossy().as_ref()).arg("-o").arg(&out)
            .arg("-lobjc").arg("-framework").arg("Foundation");
        execute_command(&mut comp, config).await
            .map_err(|e| PolyglotError::CompilationFailed(e.to_string()))?;
        let mut run = Command::new(&out);
        execute_command(&mut run, config).await
...
impl LanguageRunner for VisualBasicRunner {
    fn language_name(&self) -> &'static str { "Visual Basic" }
    fn language(&self) -> Language { Language::VisualBasic }
    fn file_extension(&self) -> &'static str { ".vb" }
    async fn run(&self, code: &str, config: &PolyglotConfig) -> PolyglotResult<String> {
        let dir = tempfile::tempdir()?;
        ...
        comp.arg(src.to_string_lossy().as_ref()).arg("-out:").arg(&out);
        execute_command(&mut comp, config).await
            .map_err(|e| PolyglotError::CompilationFailed(e.to_string()))?;
